...
/**
 * Base class for implementations of {@link org.apache.nifi.registry.security.authorization.ConfigurableAccessPolicyProvider}
 * backed by a relational database. Sub-classes must provide access to a {@link DataSource} and any identity mappings.
 *
 * NOTE: This class intentionally does not depend on anything specific from NiFi Registry so that it can move to
 * a shared security library at some point and act as a common base class for NiFi and NiFi Registry.
 */
public abstract class AbstractDatabaseAccessPolicyProvider extends AbstractConfigurableAccessPolicyProvider {

    private static final Logger LOGGER = LoggerFactory.getLogger(AbstractDatabaseAccessPolicyProvider.class);
    ...
    /**
     * Provides the DataSource required for interacting with the database.
     *
     * NOTE: This method will be called during @{method onConfigured} so the DataSource must be available at that time.
     *
     ...
     */
    protected abstract void populateInitialPolicies(final AuthorizerConfigurationContext configurationContext);
