...
If you are using database for storing buckets and flows (likes described in the 'Database Properties' section above), you can store the users, groups and policies in the same database either. 

**Note:** This UserGroupProvider is available from version 0.8.0.


The DatabaseUserGroupProvider takes the DB-connection information from `nifi-registry.properties` file, so in the `authorizers.xml` you don't need to give any addition information about the DB, you can specify initial users if you want (in the same way described did it in the `FileUserGroupProvider`):
