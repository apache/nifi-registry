...
The DatabaseUserGroupProvider takes the DB-connection information from `nifi-registry.properties` file, so in the `authorizers.xml` you don't need to give any addition information about the DB, you only need to specify the initial users (in the same way described in the `FileUserGroupProvider`).
...
* **UGP_USER_GROUP -** Stores all the matching between users and their groups, so each row contains the fields: USER_IDENTIFIER and GROUP_IDENTIFIER.
...
NOTE: Initial User Identities are only created if there are no users in the users tables in the DB or if the table is empty during NiFi Registry startup. Changes to the configured Initial Users Identities will not take effect if the DB's table is populated.
