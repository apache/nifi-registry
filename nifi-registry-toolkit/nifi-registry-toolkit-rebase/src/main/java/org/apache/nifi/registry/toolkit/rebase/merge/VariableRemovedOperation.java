...
public class VariableRemovedOperation implements MergeOperation {
    private final String name;

    public VariableRemovedOperation(String name) {
        this.name = name;
    }
    ...
    public void apply(RebaseApplicationContext rebaseApplicationContext, VersionedProcessGroup processGroup, String groupId, ComponentType componentType, String componentId) {
        VersionedComponentCollection.findById(processGroup, groupId).orElseThrow(() -> new IllegalStateException("Couldn't find process group " + groupId)).getVariables().remove(name);
    }
}
